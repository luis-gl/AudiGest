...
def generate_subject_data(csv_data: pd.DataFrame,
    ...
    for i in tqdm(range(len(csv_data))):
        ...
        melspec, mfccs = feature_extractor.get_melspec_and_mfccs(audio_path)
        save_numpy(melspec, file_name=f'{audio_fname}_{sample_rate}.npy', dir_path=melspec_path)
        save_numpy(mfccs, file_name=f'{audio_fname}_{sample_rate}.npy', dir_path=mfcc_path)

        framed_mfccs = feature_extractor.process_framed_mfcc(mfccs)
        ...
        mfcc_frames = framed_mfccs.shape[0]
        video_frames = video_landmarks.shape[0]
        assert mfcc_frames > 0 and video_frames > 0
